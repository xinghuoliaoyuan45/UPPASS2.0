...
//! (this.props.onPulling || this.props.onPullOk || this.props.onPullRelease);
// 定义onPull***属性时scrollEnabled为false
export default class extends Component {
    constructor(props) {
        ...
        this.topIndicatorHeight = this.props.topIndicatorHeight ?
            this.props.topIndicatorHeight : defaultTopIndicatorHeight;
    ...
    onPanResponderMove(e, gesture) {
        ...
        this.setState({
            progress: Math.min(1, gesture.dy / (this.topIndicatorHeight + this.pullOkMargin)),
        });
        if (isUpGesture(gesture.dx, gesture.dy)) { // 向上滑动
            ...
            } else if (this.scroll.scrollTo) {
                this.scroll.scrollTo({x: 0, y: gesture.dy * -1});
            } else {
                this.scroll.scrollToOffset({animated: false, offset: gesture.dy * -1});
            }
    ...
    // onPanResponderMoveEvent = Animated.event([{
    //     nativeEvent: {contentOffset: {dy: this.state.scrollY}},
    // }], {
    ...
    progress() {
        return this.state.scrollY.interpolate({
            inputRange: [-this.topIndicatorHeight, 0],
            outputRange: [1, 0],
            extrapolate: 'clamp',
        });
    }
    ...
    setFlag(flag) {
        if (this.flag !== flag) {
            ...
            // this.txt.setNativeProps({text: `${Math.random()}`});
            // setNativeProps可解决滑动卡顿问题，以下代码都没有解决
            // if(this.flag.pullok) {
    ...
    */
    ...
    onLayout(e) {
        if (this.state.width !== e.nativeEvent.layout.width || this.state.height
        ...
        return (
            <View style={{
                ...
                <LottieView
                    ...
                    source={require('../../assets/reload-animation.json')}
